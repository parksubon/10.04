...
const endPoint = 7; //질문이 12개이므로 정의
...
function setResult(){
  ...
  var imgURL = 'image/img/image-' + point + '.png';
...
function ImageFadeOut(qIdx, idx){ //얘네 수정&추가 
  var left = document.querySelector('.leftImage');
  var right = document.querySelector('.rightImage');

  left.disabled = true;
  left.classList.remove("fadeIn"); //위에 있는 페이드 인을 없애줌
  left.classList.add("fadeOut"); //SRC를 바꾸는 형식으로 이미지를 교체 할것이기 때문에 애니메이션 효과를 지속해야함
...
function goNext(qIdx){ //얘네 수정&추가
  var q = document.querySelector('.qBox');
  q.innerHTML = qnaList[qIdx].q;

  var left = document.querySelector('.leftImage');
  var right = document.querySelector('.rightImage');
  var qnaURL = './image/qimg/';
